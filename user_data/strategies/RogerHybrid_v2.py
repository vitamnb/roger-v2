"""
Roger Hybrid Strategy v2 - Quality
+ ENGULFING: RSI35/30, vol1.2, RR2.0, age50
Expectancy: $12.55, 61.5% win, 13 trades/6mo
"""
...
class RogerHybrid_v2(IStrategy):
    timeframe = '1h'
    stoploss = -0.02
    trailing_stop = False
    ...
    def _detect_order_blocks(self, df: DataFrame) -> DataFrame:
        ...
        for i in range(self.order_block_lookback, len(df)):
            ...
            if block_candle['close'] >= block_candle['open']: continue
            future_slice = df.iloc[i - self.order_block_lookback + 1:i + 1]
            if len(future_slice) < 2: continue
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        for i in range(1, len(dataframe)):
            ...
            if not dataframe['bullish_engulfing'].iloc[i]: continue
            ...
            if rsi_cross and ob_zone and vol_spike:
                ...
                dataframe.loc[dataframe.index[i], 'enter_tag'] = f'v2_quality|ob_low={ob_low:.4f}'
        return dataframe
    ...
    def custom_exit(self, pair, trade, current_time, current_rate, current_profit, **kwargs):
        if current_profit >= self.rr_target: return 'tp_3pct'
        return None
